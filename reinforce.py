...
class REINFORCE_Policy():  # Use to generate a policy
    ...
    def __init__(self, inChannel, lr, load=False):
        ...
        #  Neural Network use to generate the parameters of the policy (mean, var)
        self.regressor = NormalDistribParam(inChannel)
        self.optimizer = torch.optim.AdamW(
            self.regressor.parameters(), lr=self.lr)
...
class REINFORCE_Agent():
    ...
    def choose_action(self, policy: REINFORCE_Policy, state):
    ...
    def update(self, objects,  N: int, M: int, env: gym.Env, verbose=True, scheduler=False):
        ...
        policy = objects[0]  # list[REINFORCE_Policy]
        ...
        for episode in range(nEpisode+1):  # Generate trajectories
            ...
            if (episode % frequenceUpdate == 0):
                ...
                print("AVG nStep : ", np.mean(
                    [len(probs[n]) for n in range(len(probs))]))
                for n in range(len(probs)):
                    for t in range(len(probs[n])):
                        loss += -1*probs[n][t]*torch.sum(torch.tensor(
                            [rewards[n][t]*self.gamma**(t_prime-t) for t_prime in range(t, len(probs[n]))]))
                ...
                if (verbose):
                    print(
                        f"Episode : {episode}/{nEpisode} Average Reward : {np.mean(avg_rewards)}")
                probs = []
                rewards = []
                avg_rewards = []
        if verbose:
