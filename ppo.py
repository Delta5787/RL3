...
class PPO_Policy(): # Use to generate a policy
    def __init__(self, inChannel, lr):
        ...
        self.regressor = NNSingleOut(inChannel) # Neural Network to generate the mean of the distribution use to generate an action
        self.optimizer = torch.optim.AdamW(self.regressor.parameters(), lr=self.lr) # Optimizer of the network
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, 'max') # Scheduler, reduce the lr if the average episode duration stay the same for to long while testing
...
class PPO_Critic(): # Use to approximate the V function
    def __init__(self, inChannel, lr):
        ...
        self.NN = NNSingleOut(inChannel) # Neural Network that will approximate the V function
...
class PPO_Agent():       
    ...
    def update(self,objects:list[PPO_Policy,PPO_Critic], N:int, M:int, env:gym.Env, verbose=True, scheduler=False):
        # Update a new policy
        ...
        for m in range(M):
            ...
            old_discRewards = [] # Save True V(state) (discounted sum of reward) for training
            ...
            for n in range(N): # for potentiel futur multi process implementation
                ...
                for t in range(40): # For t time steps
                    ...
                    # Choose an action
                    action, p, v = self.choose_action(policy=oldPolicy, critic=critic, state=state)
                ...
                for t in range(len(t_rewards)): 
                    t_discRewards.append(sum([t_rewards[j]*self.gamma**j for j in range(t0, t+1)]))
                    if(t_isTerminante[t]):

                      t0 = t+1
                # Normalisation of the results
                t_discRewards = torch.tensor(t_discRewards)
                t_discRewards = (t_discRewards-t_discRewards.mean())/t_discRewards.std()
            ...
            old_actions = torch.squeeze(torch.stack(old_actions, dim = 0)).detach()
            old_p = torch.squeeze(torch.stack(old_p, dim = 0)).detach()
            old_discRewards = torch.squeeze(torch.stack(old_discRewards, dim = 0)).detach()
            ...
            for k in range(40): # For k epochs
                ...
                newP, newV, newEntr = self.evaluate(policy=policy, critic=critic, state=old_state, action=old_actions)

                ratio = torch.exp(newP-old_p)

                # Surrogate loss
                loss1 = ratio * old_adv
                loss2 = torch.clamp(ratio, 1-self.epsilon, 1+self.epsilon)*old_adv
                
                #               To maximize                 To minimize
                loss = -(torch.min(loss1, loss2) - self.c1*self.mse(torch.squeeze(newV), old_discRewards) + self.c2*newEntr)
            ...
            if(verbose):
                print(f"Iter [{m}/{M}] TTL reward {test_reward[-1]} Episode Duration {test_step[-1]} Loss avg {np.mean(avg_loss)}")
    ...
    def test(self, objects:list[PPO_Policy, PPO_Critic], env:gym.Env):
        # Unpack
        policy = objects[0]
        critic =  objects[1]
        ...
        while(running):
            state = torch.tensor(np.array([state]))
            action, p, v = self.choose_action(policy=policy, critic=critic, state=state)
            action = action.view(1,)
            state, reward, term, trunc, _ = env.step(action)
            s += 1
            r += reward
